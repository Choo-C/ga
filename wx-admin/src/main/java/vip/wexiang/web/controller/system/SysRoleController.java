...
import vip.wexiang.system.service.SysPermissionService;
    ...
    public R<Void> edit(@Validated @RequestBody SysRole role) {
        ...
        if (roleService.updateRole(role) > 0) {
            ...
            keys.parallelStream().forEach(key -> {
                ...
                Optional.ofNullable(loginUser).map(LoginUser::getRoles).ifPresent(v -> {
                    if (v.stream().anyMatch(r -> r.getRoleId().equals(role.getRoleId()))) {
                        try {
